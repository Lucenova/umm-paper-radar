...
const papers: Paper[] = [
  ...
  {
    id: "vtc-video-token-coding",
    index: "218",
    title: "Visual Token Coding for Video Multimodal Large Language Models",
    shortTitle: "VTC",
    date: "2026-08-28",
    category: "统一视觉 Token",
    paradigm: "I/P-frame Residual Token Coding for Video MLLMs",
    state: "冻结MLLM的连续视觉patch tokens；先预测I/P帧结构，再用帧间残差估计冗余并分配保留token",
    objective: "无需训练MLLM；DyRSO、DyTA与SC-TopK根据分辨率、帧残差和空间覆盖动态选择输入token",
    decoding: "理解侧一次性压缩视频输入，不生成像素；保留token按原模型顺序进入Qwen3-VL等MLLM",
    sharing: "不改变LLM、视觉encoder、词表或输出head；压缩发生在视觉token进入主干之前，可与IBQ/merger并行比较",
    open: "论文与仓库已公开；仓库当前仅含README，完整实现仍待补充",
    priority: "精读",
    summary: "VTC把视频压缩从逐帧Top-K改写为类似HEVC的I/P帧与残差编码：先判断哪些帧提供新信息，再在50% token预算下让Qwen3-VL平均性能保留100.1%，预算降到25%时仍保留97.8%。",
    why: "它直接补齐多帧威胁检测中的关键对照：固定2×2 folding只做空间压缩，而VTC显式利用时间冗余。两者必须分开，才能判断收益来自空间merger还是历史帧选择。",
    inspiration: "让IBQ/Qwen视觉token同时具有frame role与residual score：遮挡前身份、轨迹拐点、OCR变化和新出现目标作为I帧或高预算P帧，重复背景与静止目标只保留少量覆盖token。",
    experiment: "固定Qwen3、视觉encoder/IBQ、视频帧数、平均token预算和推理FLOPs，比较逐帧2×2、全局Top-K、VTC I/P residual、VTC+威胁/OCR salience；报告OCRBench/TextVQA、目标持续性、威胁AUPRC、轨迹误差、关键帧召回、吞吐、显存和token预算分布。",
    paper: "https://arxiv.org/abs/2608.28008",
  ...
  {
    id: "commercevibe-executable-visual-code",
    index: "219",
    ...
    shortTitle: "CommerceVibe",
    date: "2026-08-28",
    category: "自回归建模",
    paradigm: "Executable HTML/CSS Program Synthesis + Dual-Feedback RL",
    state: "Qwen3.5-9B生成的离散HTML/CSS代码token；商品图、文字、布局与样式由浏览器renderer形成最终像素",
    objective: "28K样本SFT的next-code-token CE + 规则奖励（文字/商品/布局）+ VLM视觉质量奖励",
    decoding: "严格AR生成可执行代码，再由确定性浏览器渲染；不在VAE latent、IBQ ID或像素空间采样",
    sharing: "可共享Qwen语言词表与Transformer，但不共享IBQ视觉tokenizer、image embedding或生成head；像素decoder被renderer替代",
    open: "论文和完整实验已公开；截至收录日未发现官方代码、数据、模型或项目页",
    priority: "精读",
    summary: "CommerceVibe把带大量文字和布局约束的图像生成改写为HTML/CSS程序合成：Qwen3.5-9B先SFT，再用规则与VLM双反馈RL。其1,300例加权评分由SFT的87.3提升到94.0。",
    why: "它不是IBQ/URSA/ELF的替代品，而是生成文字与可编辑版式的结构化上界：若可执行布局能稳定渲染正确文字，说明像素生成失败主要来自视觉token/decoder，而不是Qwen无法理解指令。",
    inspiration: "可让Qwen3先输出layout schema、文字字符串与bbox，再把该结构作为URSA/ELF条件；同时保留纯IBQ端到端路线，判断显式结构是否能减少文字错误与code palette负担。",
  ...
  {
    id: "densitykv-long-video-memory",
    index: "220",
    title: "DensityKV: Density-Guided KV Cache Compression for Long Video Generation",
    shortTitle: "DensityKV",
    date: "2026-08-28",
    category: "视频生成加速",
    paradigm: "Training-free Head-specific Historical KV Coreset",
    state: "每层每头独立保存已完成视频块的clean post-RoPE K/V；近期局部窗口与持久历史bank分离",
    objective: "不训练生成器或retriever；以Soft-Riesz key-space density决定历史token准入和淘汰，value保持与key精确配对",
    decoding: "原块间AR、块内video diffusion流程不变；每头bank容量有界，持久存储不随rollout长度增长",
    sharing: "不改变VAE、Transformer权重、noise/velocity目标或输出head；仅替换历史KV管理，可直接接AR-video或ELF视频分支",
    open: "Apache-2.0官方代码已公开，包含推理路径、三种backbone适配与定性案例；训练checkpoint和内部评测工具未提供",
    priority: "精读",
    summary: "DensityKV把长视频历史记忆视为每个attention head中的几何coreset：在固定9,360 tokens/head和5个近期latent帧的配置下，密度控制保留非冗余历史状态，并在Causal-Forcing、Self-Forcing和LongLive上保持有界内存。",
    why: "它说明长时一致性不能只靠增加窗口或保留更多帧；对Qwen3+IBQ多帧威胁模型，身份、OCR和关键事件可能分别由不同head维护，统一Top-K会误删稀有但关键证据。",
    inspiration: "在IBQ/ELF视频分支中分离recent KV与persistent KV，并按head审计保留的token是否对应目标身份、文字、轨迹拐点或背景锚点；记忆机制与动力学目标保持正交。",
    experiment: "固定视频backbone、NFE、局部窗口、总KV容量和rollout长度，比较FIFO、attention-score、全层共享Top-K、DensityKV与任务salience-DensityKV；报告1/8/32块身份/OCR/运动漂移、action敏感性、威胁AUPRC、VBench-Long、吞吐和峰值显存。",
  ...
  {
    id: "spatialcrafter-3d-proxy-world-model",
    index: "221",
    title: "SpatialCrafter: Single Image World Modeling with Generative 3D Proxies",
    shortTitle: "SpatialCrafter",
    date: "2026-08-28 · v2 · SIGGRAPH Asia 2026",
    category: "世界模型",
    paradigm: "3D Proxy Flow + Proxy-conditioned Video Diffusion",
    state: "单图生成的point-anchored稀疏3D proxy + proxy定义的RGB-D/视频VAE latent；几何状态与外观细节分离",
    objective: "PaSS Flow Matching预测全局3D proxy；Generative Deferred Refiner在proxy条件下恢复高频外观，配合Proxy-Aware Corruption",
    decoding: "先一次性生成持久3D proxy，再按相机轨迹分块渲染长视频；跨块复用proxy作为全局空间记忆",
    sharing: "不共享Qwen3/IBQ词表或head；可把Qwen输出的对象/深度状态接入proxy，IBQ/ELF作为可替换外观refiner",
    open: "论文、项目页和Apache-2.0官方仓库已公开；推理/训练代码、115K数据与权重仍列为待发布",
    priority: "精读",
    summary: "SpatialCrafter把单图可探索世界建模拆成两步：PaSS Flow先建立全局3D proxy，视频模型只负责proxy上的外观细化。115K场景训练后可按相机轨迹分块生成长RGB-D视频，并用持久proxy抑制快速视角变化下的漂移。",
    why: "它给当前整帧IBQ/ELF未来生成提供了更强的解耦端点：长时一致性可能来自显式全局几何，而不是更大的视频context或更复杂的velocity head。",
    inspiration: "多帧威胁模型可先维护对象、道路与相机的稀疏3D/pseudo-3D proxy；Qwen3读写语义与风险，ELF只渲染所需视角。OCR和身份纹理应留在外观流，而位置、遮挡和接触进入proxy。",
    experiment: "固定相机轨迹、视频refiner、数据、NFE和渲染预算，比较无proxy视频ELF、2D深度/track条件、一次性3D proxy和逐块更新proxy；报告camera control、3D/2D轨迹、遮挡恢复、loop closure、32块漂移、OCR/身份保真、威胁预测与延迟。",
    paper: "https://arxiv.org/abs/2608.27073",
    project: "https://fangchuan.github.io/SpatialCrafter/",
    code: "https://github.com/fangchuan/SpatialCrafter",
    codeLabel: "发布计划",
    action: "相机轨迹是主要控制；当前不包含机器人真实action、reward或value接口",
  ...
  {
    id: "pawbench-probabilistic-world-model-eval",
    index: "222",
    title: "PAWBench: How Far Are We from Probabilistically Aligned World Modeling?",
    shortTitle: "PAWBench",
    date: "2026-08-28 · v2",
    category: "评测诊断",
    paradigm: "Repeated-rollout Outcome Distribution Evaluation",
    state: "固定初始观测与动作下，重复视频rollout得到的离散物理结果及其经验概率分布",
    objective: "无训练目标；PAWEval把多次生成映射为outcome distribution，并同时评价概率匹配与有效行为覆盖",
    decoding: "兼容AR、Diffusion、Flow与混合视频生成器；同一条件重复采样而不是只选一条最好看的轨迹",
    sharing: "架构无关，不要求共享UMM tokenizer、Transformer或head；适合作为URSA/ELF世界模型的共同外部评价层",
    open: "v2论文与50场景、11个系统的完整实验已公开；截至收录日未发现官方评测代码或项目页",
    priority: "精读",
    summary: "PAWBench指出世界模型还必须匹配‘未来可能怎样发生’的概率，而不只是生成一条合理视频。对50个场景和11个系统的重复rollout中，没有模型能同时稳定恢复参考概率和有效行为范围。",
    why: "多帧威胁预测尤其需要分布校准：一次没有碰撞的漂亮rollout不能证明低风险；系统必须给出碰撞、遮挡、转向等分支概率，并在罕见高危结果上保持覆盖。",
    inspiration: "把URSA/ELF随机性转成可审计的威胁outcome分布，区分prompt、初始noise和模型训练对分布的影响；避免只汇报FVD、最好样本或单次未来准确率。",
    experiment: "固定Qwen3、IBQ、条件、采样数和总FLOPs，对AR、URSA、ELF分别重复N=8/32/128次；报告outcome calibration、Wasserstein/TV距离、mode coverage、罕见高危召回、动作置换敏感性、horizon误差、规划收益与每有效分支成本。",
...
const englishOverrides: Record<string, Partial<EnglishPaperCopy>> = {
  "vtc-video-token-coding": {
    summary: "Visual Token Coding applies I/P-frame and residual-coding ideas to video MLLMs. Its dynamic variant preserves 100.1% of Qwen3-VL's average benchmark performance at a 50% token budget and 97.8% at a 25% budget, without tuning the MLLM.",
    why: "It supplies the missing temporal-compression control for multi-frame threat analysis. Fixed 2×2 folding compresses space independently in each frame; VTC explicitly removes temporal redundancy, so the two effects can be measured separately.",
    inspiration: "Assign frame roles and residual scores to IBQ/Qwen visual tokens. Preserve pre-occlusion identity, trajectory turns, OCR changes, and newly appearing threats as I-frames or high-budget P-frames; compress repeated backgrounds aggressively.",
    experiment: "Fix Qwen3, the visual encoder or IBQ tokenizer, frame count, mean token budget, and inference FLOPs. Compare per-frame 2×2 folding, global Top-K, VTC residual coding, and VTC plus threat/OCR salience. Report OCRBench, TextVQA, identity persistence, threat AUPRC, trajectory error, key-frame recall, throughput, memory, and token allocation.",
    state: "Continuous visual patch tokens from a frozen MLLM; I/P-frame structure and frame residuals estimate redundancy.",
    objective: "Training-free dynamic resolution, token allocation, and spatial-coverage selection.",
    decoding: "Input-side video-token compression only; retained tokens enter the original MLLM in order.",
    sharing: "Leaves the vision encoder, LLM, vocabulary, and output head unchanged; directly comparable with an IBQ merger.",
    open: "Paper and repository are public; the repository currently contains only a README and the full implementation is pending.",
  },
  "commercevibe-executable-visual-code": {
    summary: "CommerceVibe represents text-heavy visual design as executable HTML/CSS program synthesis. Qwen3.5-9B is trained on more than 28,000 examples, then optimized with rule-based and VLM feedback; its weighted score rises from 87.3 after SFT to 94.0 after RL.",
    why: "It is a structured upper-bound rather than a replacement for IBQ, URSA, or ELF. If deterministic rendering preserves text and layout, raster-generation failures can be attributed more precisely to visual tokens or the decoder rather than instruction understanding.",
    inspiration: "Have Qwen3 predict a layout schema, exact strings, and bounding boxes before conditioning URSA or ELF. Retain the end-to-end IBQ route as a matched baseline to test whether explicit structure reduces glyph errors and code-palette pressure.",
    experiment: "Fix the Qwen3 backbone, prompts, data volume, and training FLOPs. Compare IBQ-AR, URSA, ELF, layout-schema-to-IBQ, and HTML/CSS rendering. Report text CER, character accuracy, layout IoU, object fidelity, GenEval, editability, latency, and failure taxonomy.",
    state: "Discrete HTML/CSS code tokens generated by Qwen3.5-9B; a browser renderer produces pixels from text, layout, and product assets.",
    objective: "Next-code-token CE on 28K SFT examples, followed by rule and VLM dual-feedback reinforcement learning.",
    decoding: "Strict AR code generation followed by deterministic browser rendering; no VAE, IBQ, or pixel-space sampling.",
    sharing: "May share the language vocabulary and Transformer, but not an IBQ tokenizer, image embedding table, or image head.",
    open: "The paper and experiments are public; no official code, data, model, or project page was found at indexing time.",
  },
  "densitykv-long-video-memory": {
    summary: "DensityKV treats long-video history as a geometric coreset in each attention head. It stores clean post-RoPE K/V states in bounded head-specific banks while preserving a short local window, and supports Causal-Forcing, Self-Forcing, and LongLive backbones.",
    why: "It shows that longer windows are not equivalent to better memory. Identity, OCR, motion turns, and background anchors may live in different heads, so a global Top-K policy can delete rare but decisive evidence.",
    inspiration: "Separate recent and persistent K/V in an IBQ/ELF video branch, then audit whether retained tokens correspond to identity, text, trajectory changes, or scene anchors. Keep memory management orthogonal to the dynamics objective.",
    experiment: "Fix the video backbone, NFE, local window, total K/V budget, and rollout length. Compare FIFO, attention-score eviction, shared Top-K, DensityKV, and task-salience DensityKV. Report 1/8/32-block identity, OCR and motion drift, action sensitivity, threat AUPRC, VBench-Long, throughput, and peak memory.",
    state: "Clean post-RoPE K/V states stored in separate per-layer, per-head historical banks plus a recent local window.",
    objective: "Training-free Soft-Riesz density controls admission and eviction; paired values are retained exactly.",
    decoding: "The original block-AR, within-block video-diffusion process is unchanged; persistent memory stays bounded with rollout length.",
    sharing: "Does not modify the VAE, Transformer weights, noise/velocity target, or output head.",
    open: "Apache-2.0 inference code, backbone adapters, and paper cases are public; training checkpoints and internal evaluation tools are omitted.",
  },
  "spatialcrafter-3d-proxy-world-model": {
    summary: "SpatialCrafter first generates a persistent, point-anchored 3D proxy with Flow Matching, then uses a proxy-conditioned video model only for appearance refinement. The proxy supports chunked long RGB-D generation under camera trajectories.",
    why: "It is a strong decoupled endpoint for full-frame IBQ/ELF futures: long-horizon consistency may come from explicit global geometry rather than a larger video context or a more complex velocity head.",
    inspiration: "Maintain a sparse 3D or pseudo-3D proxy for objects, roads, and the camera. Let Qwen3 read and update semantic/risk state, while ELF renders requested views. Keep OCR and identity texture in the appearance stream and move position, occlusion, and contact into the proxy.",
    experiment: "Fix camera trajectories, the video refiner, data, NFE, and rendering budget. Compare proxy-free video ELF, 2D depth/track conditioning, one-shot 3D proxy, and block-updated proxy. Report camera control, trajectories, occlusion recovery, loop closure, 32-block drift, OCR/identity fidelity, threat prediction, and latency.",
    state: "A point-anchored sparse 3D proxy plus proxy-conditioned RGB-D/video-VAE latents; geometry and appearance are separated.",
    objective: "PaSS Flow Matching predicts the global 3D proxy; a Generative Deferred Refiner restores high-frequency appearance with proxy-aware corruption.",
    decoding: "Generate the proxy once, then render long video chunks along camera trajectories while reusing it as spatial memory.",
    sharing: "Does not share a Qwen3/IBQ vocabulary or head; Qwen state can feed the proxy and IBQ/ELF can serve as the appearance refiner.",
    open: "Paper, project page, and Apache-2.0 repository are public; inference/training code, weights, and the 115K-scene dataset remain on the release plan.",
    action: "Camera trajectory is the main control; the current system has no real robot action, reward, or value interface.",
    rollout: "Supports long chunked video generation with persistent 3D memory, but not closed-loop action selection or MPC.",
    evaluation: "Add camera intervention, loop revisits, occlusion recovery, proxy error, and task success to visual and geometric metrics.",
  },
  "pawbench-probabilistic-world-model-eval": {
    summary: "PAWBench evaluates whether repeated video rollouts recover the probability distribution of valid physical outcomes under the same initial observation and action. Across 50 scenarios and 11 systems, no model consistently matches both reference probabilities and behavior coverage.",
    why: "Threat forecasting is distributional: one visually plausible no-collision rollout does not imply low risk. Models must cover and calibrate collision, occlusion, turning, and other rare but consequential branches.",
    inspiration: "Turn URSA/ELF sampling into an auditable threat-outcome distribution. Separate the effects of language prompts, initial noise, and training on the predicted distribution instead of reporting only a best sample or a single future.",
    experiment: "Fix Qwen3, IBQ, conditions, sample count, and total FLOPs. Run AR, URSA, and ELF for N=8/32/128 repeated rollouts; report outcome calibration, Wasserstein or total-variation distance, mode coverage, rare-threat recall, action-shuffle sensitivity, horizon error, planning gain, and cost per recovered branch.",
    state: "Empirical distributions over discrete physical outcomes obtained from repeated video rollouts under fixed observations and actions.",
    objective: "Evaluation only: PAWEval maps rollouts to outcomes and measures probability alignment plus valid-mode coverage.",
    decoding: "Architecture-agnostic; repeat sampling from AR, diffusion, flow, or hybrid generators rather than selecting one best-looking trajectory.",
    sharing: "No shared UMM tokenizer, Transformer, or head is required; it is a common external evaluation layer.",
    open: "The v2 paper and experiments over 50 scenarios and 11 systems are public; no official evaluation code or project page was found at indexing time.",
    action: "Evaluates outcome probabilities under fixed actions; it does not generate actions.",
    rollout: "Requires repeated multi-step rollouts; it provides no planner but can qualify prediction distributions before planning.",
    evaluation: "Probability calibration, mode coverage, and high-risk tails are primary; single-video FVD or LPIPS is insufficient.",
  },
    ...
    <>
      ...
      <section className="world-section" id="world-matrix">
        ...
        <div className="matrix-wrap"><table className="world-table"><thead><tr><th>Model</th><th>Observation state</th><th>Action</th><th>Dynamics target</th><th>Rollout / planning</th><th>UMM connection</th></tr></thead><tbody>
          ...
          <tr><th>SpatialCrafter</th><td>Persistent 3D proxy + video latent</td><td>Camera trajectory</td><td>Proxy Flow + appearance refinement</td><td>Chunked long-video rollout</td><td>Geometry state separated from IBQ/ELF rendering</td></tr>
          <tr><th>PAWBench</th><td>Repeated-rollout outcome distribution</td><td>Fixed action condition</td><td>Evaluation only</td><td>Repeated stochastic rollouts</td><td>Calibration and tail-risk audit for AR/URSA/ELF</td></tr>
        </tbody></table></div>
      </section>
    </>
  );
            ...
            <div className="matrix-wrap">
              <table className="world-table">
                ...
                <tbody>
                  ...
                  <tr><th>SpatialCrafter</th><td>point-anchored稀疏3D proxy + proxy-conditioned RGB-D/video latent</td><td>相机轨迹；无真实机器人action/reward/value</td><td>PaSS Flow生成proxy + Generative Deferred Refiner恢复外观</td><td>显式3D state + proxy-conditioned video diffusion</td><td>复用proxy分块长视频；未展示闭环行动或MPC</td><td>Qwen3读写几何/风险state，IBQ/ELF作为可替换外观renderer</td></tr>
                  <tr><th>PAWBench</th><td>固定观测/动作下重复rollout的outcome distribution</td><td>固定真实动作或控制条件；本身不生成动作</td><td>无训练目标；PAWEval比较概率与有效行为覆盖</td><td>兼容AR、Diffusion、Flow和混合模型</td><td>要求同条件重复rollout；不提供planner</td><td>为AR/URSA/ELF补概率校准、mode coverage与罕见高危尾部</td></tr>
